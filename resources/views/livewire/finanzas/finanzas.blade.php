    ...
    <div class="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            <div class="liquid-glass p-8 rounded-[3rem] border border-white shadow-xl group overflow-hidden relative">
                ...
                <p class="text-4xl font-black text-slate-900 tracking-tighter">${{ number_format($ingresos, 2) }}</p>
            </div>
            ...
            <div class="liquid-glass p-8 rounded-[3rem] border border-white shadow-xl group overflow-hidden relative">
                ...
                <p class="text-4xl font-black text-slate-900 tracking-tighter">${{ number_format($gastos, 2) }}</p>
            </div>

            <div class="liquid-glass p-8 rounded-[3rem] border border-white shadow-xl group overflow-hidden relative">
                @php $balance = $ingresos - $gastos; @endphp
                <div class="absolute -right-4 -top-4 w-24 h-24 {{ $balance >= 0 ? 'bg-blue-500/5' : 'bg-rose-500/5' }} rounded-full group-hover:scale-150 transition-transform duration-700"></div>
                <p class="text-[10px] font-black {{ $balance >= 0 ? 'text-blue-600' : 'text-rose-500' }} uppercase tracking-[0.2em] mb-2">Balance Operativo</p>
        ...
        <div x-show="formOpen"
            ...
            <h2 class="text-3xl font-black text-slate-900 mb-10 flex items-center gap-4 font-jakarta uppercase tracking-tighter">
                ...
                {{ $editandoId ? 'Modificar Registro Contable' : 'Nueva Transacción de Capital' }}
            </h2>
            ...
            <form wire:submit="crear" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-3">
                        ...
                        @error('categoria') <span class="text-rose-500 text-[10px] font-black uppercase ml-6">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-6 pt-6">
                    <button type="submit" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="crear">{{ $editandoId ? 'Confirmar Modificación' : 'Ejecutar Transacción' }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="space-y-3">
                    ...
                    <select wire:model.live="filtroCategoria" class="w-full px-6 py-4 bg-white/60 border-none rounded-2xl text-[10px] font-black text-slate-700 focus:ring-4 focus:ring-blue-500/10 transition-all">
                        ...
                        @foreach($categorias as $cat) <option value="{{ $cat }}">{{ strtoupper($cat) }}</option> @endforeach
                    </select>
                </div>
                <div class="space-y-3">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100/50">
                        @forelse($transacciones as $transaccion)
                            <tr wire:key="tx-{{ $transaccion->id }}" class="group hover:bg-white/80 transition-all duration-500">
                                <td class="px-10 py-8">
                                    <div class="text-xl font-bold text-slate-900 tracking-tight">{{ $transaccion->categoria }}</div>
                                    <div class="text-xs font-medium text-slate-400 mt-1 italic line-clamp-1">{{ $transaccion->descripcion ?? 'Sin glosa descriptiva' }}</div>
                                </td>
                                <td class="px-10 py-8 text-center">
                                    <span class="inline-flex px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest
                                        {{ $transaccion->tipo === 'ingreso' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-500 border border-rose-100' }}">
                                        {{ $transaccion->tipo }}
                                    </span>
                                </td>
                                <td class="px-10 py-8 text-center font-black text-lg {{ $transaccion->tipo === 'ingreso' ? 'text-emerald-600' : 'text-rose-500' }}">
                                ...
                                <td class="px-10 py-8 text-right space-x-4">
                                    <button wire:click="editar({{ $transaccion->id }})" class="p-4 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all active:scale-90">
                                    ...
                                    <button wire:click="eliminar({{ $transaccion->id }})" wire:confirm="¿Desea purgar este registro de capital?" class="p-4 bg-rose-50 text-rose-400 rounded-2xl hover:bg-rose-600 hover:text-white transition-all active:scale-90">
