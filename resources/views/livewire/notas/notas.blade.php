    ...
    <div class="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <div x-show="formOpen"
            ...
            <h2 class="text-3xl font-black text-slate-900 mb-10 flex items-center gap-4 font-jakarta">
                ...
                <span x-text=" @js($editandoId) ? 'Editando Metadata' : 'Nuevo Registro de Conocimiento' "></span>
            </h2>
            ...
            <form wire:submit="crear" class="space-y-10">
                <div class="space-y-3">
                    ...
                    @error('titulo') <span class="text-red-500 text-[10px] font-black uppercase tracking-tighter ml-6">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-6 pt-6">
                    <button type="submit" 
                        ...
                        <span wire:loading.remove wire:target="crear" x-text=" @js($editandoId) ? 'ACTUALIZAR CONOCIMIENTO' : 'PUBLICAR NOTA' "></span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                @forelse($notas as $nota)
                    <div wire:key="nota-{{ $nota->id }}" 
                        ...
                        class="liquid-glass rounded-[3rem] shadow-xl transition-all duration-500 p-10 flex flex-col border-t-[8px] {{ $nota->archivada ? 'border-slate-300 opacity-60' : 'border-indigo-400' }}"
                        ...
                        <div class="flex justify-between items-start mb-6">
                            <h3 class="text-2xl font-black text-slate-900 transition-colors tracking-tight leading-tight flex-1"
                                :class="hovered ? 'text-indigo-600' : '' ">{{ $nota->titulo }}</h3>
                        ...
                        <div class="text-slate-500 font-medium text-base mb-8 flex-1 line-clamp-5 leading-relaxed tracking-wide">
                            {{ $nota->contenido ?? 'Sin metadata adicional' }}
                        </div>

                        @if($nota->tags)
                            <div class="flex flex-wrap gap-2 mb-8">
                                @foreach(explode(',', $nota->tags) as $tag)
                                    @if(trim($tag))
                        ...
                        <div class="flex gap-3 mt-auto pt-8 border-t border-slate-100/50">
                            <button wire:click="editar({{ $nota->id }})" 
                            ...
                            @if($nota->archivada)
                                <button wire:click="desarchivar({{ $nota->id }})" 
                            ...
                            @else
                                <button wire:click="archivar({{ $nota->id }})" 
                ...
                @empty
                    <div class="col-span-full text-center py-40 bg-white/40 rounded-[4rem] border-4 border-dashed border-slate-200">
                        ...
                        <p class="text-slate-400 mt-4 text-lg font-medium max-w-md mx-auto leading-relaxed">
                            {{ $busqueda ? 'No hay registros que coincidan con los parámetros de búsqueda.' : 'Inicie la documentación de su proyecto creando la primera nota corporativa.' }}
                        </p>
                        @if($busqueda)
